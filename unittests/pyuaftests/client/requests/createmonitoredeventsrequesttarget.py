...
class CreateMonitoredEventsRequestTargetTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # BUG in SWIG???
        # create EventFilter in a separate step and assign it to self.target1.eventFilter!
        # if you simply do self.target1.eventFilter.selectClauses.resize(1), Swig 3.0.5 segfaults!
        eventFilter = pyuaf.util.EventFilter()
        eventFilter.selectClauses.resize(1)
        eventFilter.selectClauses[0].indexRange = "indexRange"
        self.target1.eventFilter = eventFilter
